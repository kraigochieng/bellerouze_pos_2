...
def read_item_categories(db: Session, skip: int = 0, limit: int = 10):
    return db.query(models.ItemCategory).offset(skip).limit(limit).all()
...
def read_item_with_category(db: Session, id: int):
    return db.query(models.ItemWithCategory).filter(models.ItemWithCategory.id == id).first()


def read_item_with_categories(db: Session, skip: int = 0, limit: int = 10):
    return db.query(models.ItemWithCategory).offset(skip).limit(limit).all()

def update_item_with_category(db: Session, id: int, update_model=schemas.ItemWithCategoryUpdate):
